...
{
    Task<ApiKey> CreateAsync(int userId, string name, ApiKeyGenerationResult keyInfo, DateTime? expiresAt = null, CancellationToken ct = default);
    Task<ApiKey?> FindByHashAsync(string keyHash, CancellationToken ct = default);
    Task<System.Collections.Generic.List<ApiKey>> GetByUserIdAsync(int userId, bool includeRevoked = false, CancellationToken ct = default);
